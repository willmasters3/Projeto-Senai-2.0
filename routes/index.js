...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Agora, vamos buscar as associações na tabela ProfessorUnidade
        ...
        // Redirecionamento baseado no tipo de usuário (ou qualquer lógica desejada)
        if (user.permissao === 'admin') {
            return res.send('/html/dashboard.html'); 
        } else {
            return res.send('/html/agendasala.html');
        }
    } catch (err) {
...
/* Rota para obter tipos de aula
router.get('/listar-tipos-aula', async (req, res) => {
    try {
        const result = await pool.request().query('SELECT id_tipo_aula, descricao FROM tipos_aula');
        res.json(result.recordset);
    } catch (err) {
...
router.post('/adicionar-unidade-curricular', async (req, res) => {
    ...
    try {
        await pool.request()
            ...
            .input('semestre', sql.Nvarchar, semestre) // Mudar para Int
