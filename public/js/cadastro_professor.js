...
async function carregarUnidades() {
    ...
    try {
        ...
        if (userInfo.permissao === 'admin') {
            ...
            // Caso contrário, carrega apenas a unidade do usuário
            unidades.forEach(unidade => {
                if (unidade.codigo === userInfo.codigo_unidade) { // Filtra apenas a unidade do usuário
                    const option = document.createElement('option');
                    option.value = unidade.codigo;
                    option.textContent = unidade.nome;
                    unidadeSelect.appendChild(option);
...
document.getElementById('professorForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            alert('Funcionário/Professor adicionado com sucesso!');
            e.target.reset(); // Reinicia o formulário após adicionar o professor
                ...
                professores.forEach(professor => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            ...
                            <button onclick="excluirProfessor(${professor.id_professor})">Excluir</button>
