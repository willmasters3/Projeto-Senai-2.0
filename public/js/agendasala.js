...
async function carregarAgendamentos() {
    ...
    const mesAtual = agora.getMonth(); // Meses variam de 0 (Janeiro) a 11 (Dezembro)
    const anoAtual = agora.getFullYear();

    // Cria limites de comparação
    const primeiroDiaDoMesAtual = new Date(anoAtual, mesAtual, 1); // Primeiro dia do mês atual
    const primeiroDiaDoProximoMes = new Date(anoAtual, mesAtual + 1, 1); // Primeiro dia do próximo mês
    ...
    agendamentosAtual = agendamentosAtual.filter(agendamento => {
        ...
        return dataAgendamento >= primeiroDiaDoMesAtual && dataAgendamento < primeiroDiaDoProximoMes; 
    });
...
document.getElementById('aplicarFiltros').addEventListener('click', async () => {
    ...
    // Ordena os agendamentos filtrados por data e hora
...
function exibirAgendamentos() {
    ...
    cabecalho.innerHTML = `Agendamentos de ${mesAtual} ${anoAtual}`;
    agendamentosList.appendChild(cabecalho);
    ...
    const tabela = `
        <table class="table table-striped">
            ...
            <tbody>
                ${agendamentosParaExibir.length === 0 ? '<tr><td colspan="7" class="text-center">Nenhum agendamento encontrado para este mês.</td></tr>' : ''}
    ...
    agendamentosList.innerHTML += tabela; // Adiciona a tabela abaixo do cabeçalho
