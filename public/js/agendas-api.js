...
// A função `aplicarFiltros` continua, mas não deve incluir a lógica de exclusão e edição.
async function aplicarFiltros() {
    ...
    try {
        ...
        agendamentos.sort((a, b) => {
            const dataA = new Date(a.data_reservas + 'T' + a.hora_inicio); // Combine data e hora
...
function verificarHorarioTurno(agendamentoData, filtroTurno) {
    ...
    if (filtroTurno === 'manha') {
        return agendamentoData >= turnoManhaInicio && agendamentoData <= turnoManhaFim;
    } else if (filtroTurno === 'tarde') {
        return agendamentoData >= turnoTardeInicio && agendamentoData <= turnoTardeFim;
    } else if (filtroTurno === 'noite') {
        return agendamentoData >= turnoNoiteInicio && agendamentoData <= turnoNoiteFim;
    }
